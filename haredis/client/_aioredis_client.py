...
class AioRedisClient(aioredis.ConnectionPool):
    """
    Aio Redis Client Implementation with Sync API. It inherits from main raioedis.StrictRedis class and it has extra implementations.
    """
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        try:
            self.client_conn = aioredis.Redis(connection_pool=self)
        except aioredis.RedisError as e:
            print("AioRedis Client Initialization Error: ", e)
        
    async def connection_test(self, *args, **kwargs):
        """Main client connection test method."""
        
        try:
            _ = await self.client_conn.ping()
            print(f"Ping Response on AioRedis Client: {_}")
            print(f"Connected to Redis with async API: {kwargs.get('host', 'localhost')}:{kwargs.get('port', 6379)}.")
        except aioredis.RedisError as e:
    ...
    async def get_aioredis_client(self):
        """Get AioRedis Client as Connection Pool."""
        return self
