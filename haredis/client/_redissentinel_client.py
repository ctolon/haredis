...
class RedisSentinelClient(sentinel.SentinelConnectionPool):
    ...
    def connect_client(self, *args, **kwargs):
        ...
        try:
            host, port = self.sentinel_conn.discover_master(kwargs.get("service_name"))
            self.client_conn = redis.StrictRedis(*args, **kwargs)
            _ = self.client_conn.ping()
            print(f"Ping Response on Redis Client: {_}")
            print("Connected to Redis Sentinel with sync API.")
    ...
    def get_redis_sentinel_client(self):
        """Get Redis Sentinel Client as Connection pool."""
